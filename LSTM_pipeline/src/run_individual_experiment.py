...
def main():
    """ The pipeline to perform experiments 
        ...
        It will then perform the following steps:
            ...
            CSV file
            3. Initialize the parameters for the baseline and
            NILM models:
                3a. If load_params was specified, 
                the params will be loaded from files (JSON files)
            ...
            4. Initialize the baseline and NILM models:
                4a. If load_model was specified, the models will be 
                loaded from files (.pt files)
                4b. If load_model was not specified, 
                the models will be trained on the training 
                and validation data
            5. Perform testing on the baseline and NILM models 
    ...
    if (args.load_params):
        ...
        print(
            "computing model parameters from hyper parameter tuning for baseline...."
        )
        ...
        print("Done....")
        print(
            "Computing model parameters from hyper parameter tuning for NILM...."
        )
        params_NILM = hyper_parameter_tuning(
            training,
            validation,
            min_max_dict,
            args.out_path,
    ...
    if (args.load_models):
        ...
        model_NILM, NILM_val_loss = train_model(
            params_NILM,
            training,
            validation,
            min_max_dict,
